...
def contour_draw() :
    ...
    grayimg = cv2.bilateralFilter(origin_img, 20, 200, 200)
    grayimg = cv2.cvtColor(grayimg, cv2.COLOR_BGR2GRAY)
    
    #thresh = 35
    _, thr = cv2.threshold(grayimg, 200, 255, cv2.THRESH_BINARY)

    #canny_img = cv2.Canny(grayimg, 50, 100, L2gradient=True)
    
    #thr = cv2.adaptiveThreshold(grayimg, 200, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 255, 5)
    # blocksize % 2 ==1
    ...
    cv2.drawContours(origin_img, origin_contours, -1, (255, 0, 0), 10)
